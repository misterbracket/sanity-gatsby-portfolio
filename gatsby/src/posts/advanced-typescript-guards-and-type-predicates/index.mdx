...
date: 2022-01-04
description: "Guards and Type Predicates in TypeScript"
---

TypeScript is amazing in inferring what that type of a given object is. However, sometimes we pass more generic types like `Animal` to a function, but then then in the function we access properties that are not on all permutaion of `Animal`. We can use guards to make sure that we are operating on an object with all the required properties.
...
## Solution 1: The <span style="min-width: 8px;display: inline-block;"></span>`in` <span style="min-width: 8px;display: inline-block;"></span> Keyword
